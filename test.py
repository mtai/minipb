...
TEST_RAW_DECODED = (
    {"data": 123, "id": 1, "wire_type": 0},
    {"data": b"test", "id": 2, "wire_type": 2},
    {"data": b"\n\x06string\x10\xf8\x06", "id": 3, "wire_type": 2},
    {"data": b"\n\x0eanother_string\x10\xb9`", "id": 3, "wire_type": 2},
)

TEST_FIELD_SEEK_SIMPLE = b"\x10\x01\x18\x02R\x05test1\xa2\x01\x05test2"

TEST_FIELD_SEEK_COMPLEX = (
    b"\xa2\x01\t\x08\x02R\x05hello\xf2\x01\x06\x12\x04str1\xf2\x01\x06\x12\x04str2"
)
...
class TestMiniPB(TestCase):
    ...
    def test_badbehavior_missing_field_kvfmt(self):
        ...
        with self.assertRaises(minipb.CodecError) as details:
            w.encode({"field2": 123})
        self.assertIn(
            "empty field field1 not padded with None", details.exception.args[0]
        )
    ...
    def test_msg_init_to_dict_very_complex(self):
        ...
        test_msg = TestMessage(
            ...
            string="test",
            nested=[
                TestMessage.NestedMessage(**nested_dict)
                for nested_dict in raw_obj["nested"]
            ],
        )
    ...
    def test_ooo_field_numbers(self):
        self.assertRaises(
            AssertionError, minipb.Field, minipb.MIN_FIELD_NUMBER - 1, minipb.TYPE_SINT
        )
        self.assertRaises(
            AssertionError, minipb.Field, minipb.MAX_FIELD_NUMBER + 1, minipb.TYPE_SINT
        )

        self.assertRaises(
            AssertionError,
            minipb.Field,
            minipb.MIN_RESERVED_BY_PROTOBUF_FIELD_NUMBER,
            minipb.TYPE_SINT,
        )
        self.assertRaises(
            AssertionError,
            minipb.Field,
            minipb.MIN_RESERVED_BY_PROTOBUF_FIELD_NUMBER + 10,
            minipb.TYPE_SINT,
        )
        self.assertRaises(
            AssertionError,
            minipb.Field,
            minipb.MAX_RESERVED_BY_PROTOBUF_FIELD_NUMBER,
            minipb.TYPE_SINT,
        )
    ...
    def test_non_consecutive_field_numbers(self):
        ...
        decode_raw_objs = [
            ...
            dict(
                id=10,
                wire_type=minipb._WIRE_TYPE_I64,
                data=minipb._encode_scalar_to_bytes(
                    minipb.TYPE_DOUBLE, expected_msg.f_i64
                ),
            ),
            dict(id=777, wire_type=minipb._WIRE_TYPE_LEN, data=expected_msg.f_len),
            dict(
                id=888,
                wire_type=minipb._WIRE_TYPE_I32,
                data=minipb._encode_scalar_to_bytes(
                    minipb.TYPE_FIXED32, expected_msg.f_i32
                ),
            ),
            dict(
                id=minipb.MAX_FIELD_NUMBER,
                wire_type=minipb._WIRE_TYPE_VARINT,
                data=int(expected_msg.f_max),
            ),
