...
if _IS_MPY:
    # MicroPython re hack
    def _get_length_of_match(match):
...
def _vint_2sctosigned(
    number, max_bits=_DEFAULT_VINT_2SC_MAX_BITS, mask=_DEFAULT_VINT_2SC_MASK
):
    """
...
def _encode_fixed_length_numerical(
    field_type, py_data, mask=None
):  # pylint: disable=unused-argument
    return struct.pack(f"<{field_type}", py_data)


def _decode_fixed_length_numerical(
    field_type, field_bytes, max_bits=None, mask=None
):  # pylint: disable=unused-argument
    return struct.unpack(f"<{field_type}", field_bytes)[0]
...
_TYPE_TO_ENCODER_MAP = {
    ...
    TYPE_INT: lambda t, py_data, mask: _encode_vint(
        _vint_signedto2sc(py_data, mask=mask)
    ),
...
_TYPE_TO_DECODER_MAP = {
    ...
    TYPE_INT: lambda t, f_data, max_bits, mask: _vint_2sctosigned(
        f_data, max_bits=max_bits, mask=mask
    ),
...
def _decode_scalar_from_bytes(
    field_type, f_data, max_bits=_DEFAULT_VINT_2SC_MAX_BITS, mask=_DEFAULT_VINT_2SC_MASK
):
    """
...
def _check_bytes_length(data, length):
    if not hasattr(data, "decode"):
        raise ValueError(f"Excepted a bytes object, not {type(data).__name__}")
    if len(data) != length:
        raise ValueError(f"Excepted a bytes object of length {length}, got {len(data)}")
    return data
...
def _read_fixed(buf, length):
    ...
    if actual != length:
        raise EndOfMessage(
            False if actual == 0 else True  # pylint: disable=simplifiable-if-expression
        )
    return result
...
def _yield_fields_from_wire(buf, wire_type=None, field_number=None):
    ...
    while 1:
        ...
        except EndOfMessage as eom:
            if wire_type is None or eom.partial:
                raise CodecError(
                    f"Unexpected end of message while decoding field {field_number}"
                ) from eom
            break
...
def encode_raw(objs):
    ...
    for current_obj in objs:
        wire_type = current_obj["wire_type"]
        encoded.write(_encode_header(wire_type, current_obj["id"]))
        current_encoder = _WIRE_TYPE_TO_ENCODER_MAP.get(wire_type)
        if not current_encoder:
            raise ValueError(f"Unknown type {wire_type}")
        encoded.write(current_encoder(current_obj["data"]))
...
class _OverlapCheck:  # pylint: disable=too-few-public-methods
    """
    ...
    def add_field(self, parsed_list, parsed_field):
        ...
        if not success:
            ...
            repeats_str = (
                "" if repeats == 1 else f" or {repeats - 1} more fields after it"
            )
            name_str = "" if name is None else f" ({name})"
            raise BadFormatString(
                f"Multiple definitions found for field {start_field_id}{repeats_str}{name_str}."
            )
...
def _concat_fields(fields):
    ...
    for field in fields:
        assert (
            field["id"] == result["id"] and field["wire_type"] == result["wire_type"]
...
class Wire:  # pylint: disable=missing-class-docstring
    # Field aliases
    _FIELD_ALIAS = {
        "v": TYPE_SINT,
        "V": TYPE_UINT,
    ...
    _T_FMT = re.compile(
        r"^(?:({0})|({1}))(\d*)(?:@(\d+))?".format(  # pylint: disable=consider-using-f-string
            "|".join(_TYPE_TO_WIRE_TYPE_MAP.keys()), "|".join(_FIELD_ALIAS.keys())
        )
    )
        ...
        for entry in fmtlist:
            ...
            if isinstance(fmt, str):
                ...
                if m_prefix:
                    ...
                    # check for optional nested struct start (required if field is repeated)
                    if m_prefix.group(2):
                        if len(entry) <= 1:
                            raise BadFormatString(
                                "Nested field type used without specifying field format."
                            )
                ...
                if m_fmt:
                    ...
                    if m_fmt.group(3) and resolved_fmt_char == "x":
                        ...
                        raise BadFormatString(
                            "Field copying is not allowed in key-value format list."
                        )
                    else:
                        field_id += 1
                else:
                    raise BadFormatString(f'Invalid type for field "{name}"')
                if len(fmt) != ptr:
                    raise BadFormatString(
                        f'Unrecognized fragment "{fmt[ptr:]}" in format string'
                    )
            else:
    ...
    def _parse_format_string(
        ...
        Format of parsed format list:
            ...
            - prefix: Prefix of the field. (required, repeated, packed-repeated)
                    (EXCLUDES nested structs) Needs to be an empty string when there's none.
            - subcontent: Optional. Used for nested structs. (field_type must be `a' when defined)
        ...
        while ptr < length:
            ...
            if m_prefix:
                ...
                if m_prefix.group(2):
                    ...
                    # bracket not match
                    if not brace_offset:
                        raise BadFormatString(f"Unmatched brace on position {ptr}")
            ...
            if m_fmt:
                ...
                # fmt is an alias
                if m_fmt.group(2):
                    parsed["field_type"] = self._FIELD_ALIAS[m_fmt.group(2)]
                # fmt is an actual field type
                elif m_fmt.group(1):
                ...
                raise BadFormatString(f"Invalid token on position {ptr}")
    ...
    def _parse_msg_class(self, msg_class):
        ...
        - field_type: Type of the field. (see the doc, _FIELD_WIRE_TYPE and _FIELD_ALIAS)
        - prefix: Prefix of the field. (required, repeated, packed-repeated)
                    (EXCLUDES nested structs) Must be empty string when none.
        - subcontent: Optional. Used for nested structs. (field_type must be `a' when defined)
        - repeat: Optional. Copy this field specified number of times to consecutive indices.
        """
        parsed_list = []
        ...
        for field_name, current_field_instance in name_to_fields_map.items():
            ...
            parsed_field[
                "subcontent"
            ] = subcontent  # Set subcontent to the nested Message class
            parsed_list.append(parsed_field)
    ...
    @classmethod
    def encode_raw(cls, stuff):  # pylint: disable=missing-function-docstring
        return encode_raw(stuff)

    def _encode_wire(
        self, stuff, fmtable=None
        ...
        for fmt in fmtable:
            if self._kv_fmt or self._msg_cls:
                assert (
                    "name" in fmt
                ), "Encoder is in key-value mode but name is undefined for this field"
                stuff_id = fmt["name"]
            ...
            for field_id in range(field_id_start, field_id_start + repeat):
                try:
                    ...
                    raise CodecError(
                        ...
                        f"(empty field {name_str} not padded with None)"
                    ) from err
                ...
                if prefix == PREFIX_REQUIRED and field_data is None:
                ...
                if prefix == PREFIX_REPEATED:
                    for obj in field_data:
                        ...
                        encoded.write(self._encode_field(field_type, obj, subcontent))
    ...
    def _encode_field(self, field_type, field_data, subcontent=None):
        ...
        # everything else
            ...
            field_encoded = _encode_scalar_to_bytes(
                field_type, field_data, mask=self._vint_2sc_mask
            )
    ...
    @classmethod
    def decode_raw(cls, data):  # pylint: disable=missing-function-docstring
    ...
    def _decode_field(self, field_type, field_data, subcontent=None):
        ...
        if wt_schema != wt_data:
            raise TypeError(
                f"Wire type mismatch (expect {wt_schema} but got {wt_data})"
            )
        ...
        if field_type == TYPE_BYTES and subcontent:
            if self._msg_cls:
                ...
                field_decoded = self._decode_wire(io.BytesIO(field_bytes), subcontent)
                if self._kv_fmt:
            ...
            field_decoded = _decode_scalar_from_bytes(
                field_type,
                field_bytes,
                max_bits=self._vint_2sc_max_bits,
                mask=self._vint_2sc_mask,
            )

        return field_decoded

    def _decode_wire(
        self, buf, subfmt=None
        ...
        for fmt in subfmt:
            ...
            if self._kv_fmt or self._msg_cls:
                assert (
                    repeat == 1 or field_type == TYPE_EMPTY
                ), "Refuse to do field copying on non-skip field in key-value mode."
            ...
            for field_id in range(field_id_start, field_id_start + repeat):
                ...
                if fields is None:
                    ...
                    if field_prefix == PREFIX_REQUIRED:
                        raise CodecError(f"Field {field_id} is required but is empty")
                ...
                # normal repeated fields
                    ...
                    field_decoded = tuple(
                        self._decode_field(field_type, f, subcontent) for f in fields
                    )
                ...
                # packed repeated field
                    ...
                    if fields[0]["wire_type"] != _WIRE_TYPE_LEN:
                        ...
                        raise CodecError(
                            f"Packed repeated field {name_str} has wire type other than str"
                        )
                ...
                # not a repeated field but has multiple data in one field
                    ...
                    self.logger.warning("Multiple data found in a non-repeated field.")
                    ...
                    if subcontent is None:
                        ...
                        #  same field, as if with the `Message::MergeFrom` method – that is, all
                        # singular scalar fields in the latter instance replace those in the
                        # former, singular embedded messages are merged, and repeated fields are
                        # concatenated.
                        field_decoded = self._decode_field(
...
class Field:  # pylint: disable=too-few-public-methods
    ...
    def __init__(
        self, number, type_, required=False, repeated=False, repeated_packed=False
    ):  # pylint: disable=too-many-arguments
        assert (
            MIN_FIELD_NUMBER <= number <= MAX_FIELD_NUMBER
        ), f"Field number out of bounds, not between {MIN_FIELD_NUMBER} and {MAX_FIELD_NUMBER}"

        assert not (
            MIN_RESERVED_BY_PROTOBUF_FIELD_NUMBER
            <= number
            <= MAX_RESERVED_BY_PROTOBUF_FIELD_NUMBER
        ), (
            "Field number in Protocol Buffer reserved range of "
            f"{MIN_RESERVED_BY_PROTOBUF_FIELD_NUMBER} to {MAX_RESERVED_BY_PROTOBUF_FIELD_NUMBER}"
        )

        assert type_ in TYPES or issubclass(type_, Message)

        assert (
            sum([required, repeated, repeated_packed]) <= 1
        ), "Can only speciffy 1 of required, repeated or repeated_Packed"
...
def process_message_fields(cls):
    """
    Class decorator used to detect all Fields then track them in
        cls.{_MESSAGE_NAME_TO_FIELDS_MAP}
        cls.{_MESSAGE_NUMBER_TO_FIELDS_MAP}
    """
    # Identify all Fields
...
def _msg_inner_from_dict(in_value, current_field):
    ...
    if type(in_value) in (list, tuple):
        return tuple(
            _msg_inner_from_dict(current_value, current_field)
            for current_value in in_value
        )
    if is_message(field_type):
...
class Message:
    """
    Convenience class used to group Fields from the same schema

    Example usage (in conjunction with process_message_fields)
    ...
    def __init__(self, **kwargs):
        ...
        assert name_to_fields_map is not None, (
            f"Missing self.{_MESSAGE_NAME_TO_FIELDS_MAP}, "
            "forget to decorate Message with @process_message_fields?"
        )
    ...
    def __repr__(self):
        ...
        return f"{type(self).__name__}({1})".format(
            ", ".join(f"{key}={repr(getattr(self, key))}" for key in keys),
        )
    ...
    def to_dict(self, dict_factory=collections.OrderedDict):
        """
        Translate this Message to a dict, preserving field ordering
        """
    ...
    def from_dict(cls, in_dict):
        """
        Create a Message from a provided dict
        """
    ...
    def decode(cls, in_bytes):
        """
        Create a Message from a PB string
        """
    ...
    def wire(cls):
        """
        Lazily define a Wire instance based on this class
        """
...
if __name__ == "__main__":
    ...
    def usage():
        """Isn't that obvious?"""
        print(f"Usage: {sys.argv[0]} <-d|-e> <fmtstr>")
        sys.exit(1)
